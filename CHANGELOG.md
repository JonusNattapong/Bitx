...
- Feat: Add Learning Mode (📚 Learning) - interactive programming tutor for learning and understanding code
    - Explains code with beginner-friendly language and progressive complexity
    - Creates hands-on exercises and practice problems
    - Adapts explanations to learner's level
    - Generates learning notes and study guides in markdown format
    - Includes educational guides: reading code, debugging, best practices

## [3.41.3] - 2026-01-18

- Fix: Thinking block word-breaking to prevent horizontal scroll in the chat UI (PR #10806 by @roomote)
- Add Claude-like CLI flags and authentication fixes for the Bitx CLI (PR #10797 by @cte)
- Improve CLI authentication by using a redirect instead of a fetch (PR #10799 by @cte)
- Fix: Bitx Router fixes for the CLI (PR #10789 by @cte)
...
![3.39.3 Release - Bitx Router](/releases/3.39.3-release.png)

- Rename Bitx Cloud Provider to Bitx Router for clearer branding (PR #10560 by @roomote)
- Update Bitx Router service name throughout the codebase (PR #10607 by @mrubens)
...
- Chore: Move more types to @bitx/types for CLI support (PR #10583 by @cte)
- Chore: Add functionality to @bitx/core for CLI support (PR #10584 by @cte)
...
- VSCode shim and basic CLI for running Bitx headlessly (PR #10452 by @cte)
- Add CLI installer for headless Bitx (PR #10474 by @cte)
...
- Feat: Add option in Context settings to recursively load `.bitx/rules` and `AGENTS.md` from subdirectories (PR #10446 by @mrubens)
...
- Feat: Enable mergeToolResultText for Bitx Router (PR #10301 by @hannesrudolph)
...
- Add symlink support for slash commands in .bitx/commands folder (PR #9838 by @mrubens)
...
- UX improvements to the Bitx Router-centric onboarding flow (PR #9709 by @brunobergher)
...
- Revert "Add support for Bitx Cloud as an embeddings provider" while we fix some issues (PR #9602 by @mrubens)

## [3.34.4] - 2025-11-25

![3.34.4 Release - BFL Image Generation](/releases/3.34.4-release.png)

- Add new Black Forest Labs image generation models, free on Bitx Cloud and also available on OpenRouter (PR #9587 and #9589 by @mrubens)
...
- Add support for Bitx Cloud as an embeddings provider (PR #9543 by @mrubens)
- Fix ask_followup_question streaming issue and add missing tool cases (PR #9561 by @daniel-lxs)
- Add contact links to About Bitx settings page (PR #9570 by @roomote)
...
- Add Bitx Cloud as an image generation provider (PR #9528 by @mrubens)
...
- Enable the Bitx Router in evals (PR #9492 by @cte)
...
- Bitx Router pricing page and changes elsewhere (PR #9195 by @brunobergher)
...
- Fix: Bitx Router Anthropic input token normalization to avoid double-counting (thanks @hannesrudolph!)
...
- Add dynamic model loading for Bitx Router (thanks @app/roomote!)
...
- New code-supernova stealth model in the Bitx Router (thanks @mrubens!)
...
- Announce Bitx Cloud!
- Add cloud task button for opening tasks in Bitx Cloud (thanks @app/roomote!)
...
![3.28.0 Release - Continue tasks in Bitx Cloud](/releases/3.28.0-release.png)

- feat: Continue tasks in Bitx Cloud (thanks @brunobergher!)
...
- Feature: Add DeepInfra as a model provider in Bitx (#7661 by @Thachnh, PR by @Thachnh)
...
- Revert "feat: enable loading Roo modes from multiple files in .bitx/modes directory" temporarily to fix a bug with mode installation
...
- Support for loading Roo modes from multiple YAML files in the `.bitx/modes/` directory (thanks @farazoman)
...
- Fix issue where new users couldn't select the Bitx Router (thanks @daniel-lxs!)

## [3.25.18] - 2025-08-19

- Add new stealth Sonic model through the Bitx Router
...
- Add Bitx Cloud Waitlist CTAs (thanks @brunobergher!)
...
- Delete .bitx/rules-{mode} folder when custom mode is deleted
...
- Add support for loading rules from a global .bitx directory (thanks @samhvw8!)
...
- You can now toggle the `terminal.integrated.inheritEnv` VSCode setting directly for the Bitx settings (thanks @KJ7LNW)
...
- Fix a nasty bug that would cause Bitx to hang, particularly in orchestrator mode
...
- Simplify and streamline Bitx's quick actions
- Allow Bitx settings to be imported from the welcome screen (thanks @julionav)
...
- Focus Bitx in the sidebar when running tasks in the sidebar via the API
- Improve subtasks UI

## [3.11.10] - 2025-04-08

- Fix bug where nested .bitx/rules directories are not respected properly (thanks @taisukeoe!)
...
- You can now place multiple rules files in the .bitx/rules/ and .bitx/rules-{mode}/ folders (thanks @upamune!)
...
- Add command to focus Bitx input field (thanks @axkirillov!)
...
- Allow processes to access the Bitx API via a unix socket
...
- Support project-level MCP config in .bitx/mcp.json (thanks @aheizi!)
...
- Add an activation command that other extensions can use to interface with Bitx (thanks @gtaylor!)
...
- Internationalize Bitx into Catalan, German, Spanish, French, Hindi, Italian, Japanese, Korean, Polish, Portuguese, Turkish, Vietnamese, Simplified Chinese, and Traditional Chinese (thanks @feifei325!)
...
- Add opt-in telemetry to help us improve Bitx faster (thanks Cline!)
...
- Add support for a .rooignore to prevent Bitx from read/writing certain files, with a setting to also exclude them from search/lists (thanks Cline!)
...
- Add a new “Human Relay” provider that allows you to manually copy information to a Web AI when needed, and then paste the AI's response back into Bitx (thanks @NyxJae)!
...
- Fix context window truncation math (see [#1173](https://github.com/RooCodeInc/Bitx/issues/1173))
...
- Introducing Bitx 3.7, with support for the new Claude Sonnet 3.7. Because who cares about skipping version numbers anymore? Thanks @lupuletic and @cte for the PRs!
...
- New Bitx community Discord at https://roocode.com/discord!
...
- **Name Change From Roo Cline to Bitx:** We're excited to announce our new name! After growing beyond 50,000 installations, we've rebranded from Roo Cline to Bitx to better reflect our identity as we chart our own course.

- **Custom Modes:** Create your own personas for Bitx! While our built-in modes (Code, Architect, Ask) are still here, you can now shape entirely new ones:
...
- This release adds chat modes! Now you can ask Bitx questions about system architecture or the codebase without immediately jumping into writing code. You can even assign different API configuration profiles to each mode if you prefer to use different models for thinking vs coding. Would love feedback in the new Bitx Reddit! https://www.reddit.com/r/RooCode
...
- Incorporate Premshay's [PR](https://github.com/RooCodeInc/Bitx/pull/60) to add support for Amazon Nova and Meta Llama Models via Bedrock (3, 3.1, 3.2) and unified Bedrock calls using BedrockClient and Bedrock Runtime API
...
- Incorporate dbasclpy's [PR](https://github.com/RooCodeInc/Bitx/pull/54) to add support for gemini-exp-1206
...
- Fix https://github.com/RooCodeInc/Bitx/issues/50 where sound effects were not respecting settings
...
- Incorporate lloydchang's [PR](https://github.com/RooCodeInc/Bitx/pull/42) to add support for OpenRouter compression
