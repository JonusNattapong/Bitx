...
export default defineConfig(({ mode }) => {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("Bitx"),
		...(gitSha ? { "process.env.PKG_SHA": JSON.stringify(gitSha) } : {}),
	}

	// TODO: We can use `@bitx/build` to generate `define` once the
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("Bitx-Nightly")
	}
	...
	return {
		...
		resolve: {
			alias: {
				...
				"@bitx": resolve(__dirname, "../src/shared"),
			},
		},
		build: {
			outDir,
			...
			rollupOptions: {
				...
				// dynamically imported by bitx-config/index.ts, but should never be bundled
