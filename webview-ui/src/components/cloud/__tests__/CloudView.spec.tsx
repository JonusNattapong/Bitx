	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"cloud:signIn": "Connect to Bitx Cloud",
				"cloud:cloudBenefitsTitle": "Try Bitx Cloud",
				...
				"cloud:visitCloudWebsite": "Visit Bitx Cloud",
				"cloud:taskSync": "Task sync",
				"cloud:taskSyncDescription": "Sync your tasks for viewing and sharing on Bitx Cloud",
				...
				"cloud:remoteControlDescription":
					"Enable following and interacting with tasks in this workspace with Bitx Cloud",
				...
				"cloud:cloudUrlPillLabel": "Bitx Cloud URL: ",
			}
...
describe("CloudView", () => {
	it("should display benefits when user is not authenticated", () => {
		...
		expect(screen.getByRole("heading", { name: "Try Bitx Cloud" })).toBeInTheDocument()
	...
	it("should display remote control toggle when user has extension bridge enabled and roomote control enabled", () => {
		...
		expect(
			screen.getByText("Enable following and interacting with tasks in this workspace with Bitx Cloud"),
		).toBeInTheDocument()
	})
	...
	it("should not display cloud URL pill when pointing to production", () => {
		...
		expect(screen.queryByText(/Bitx Cloud URL:/)).not.toBeInTheDocument()
	})
	...
	it("should display cloud URL pill when pointing to non-production environment", () => {
		...
		expect(screen.getByText(/Bitx Cloud URL:/)).toBeInTheDocument()
