...
import type { AuthState } from "@bitx/types"
...
vi.mock("../core/webview/ClineProvider", async () => {
	const { BridgeOrchestrator } = await import("@bitx/cloud")
	const mockInstance = {
...
describe("extension.ts", () => {
	...
	test("authStateChangedHandler calls BridgeOrchestrator.disconnect when logged-out event fires", async () => {
		const { CloudService, BridgeOrchestrator } = await import("@bitx/cloud")
