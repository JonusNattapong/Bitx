...
describe("generateErrorDiagnostics", () => {
	...
	it("generates a diagnostics file with error metadata and history", async () => {
		...
		expect(result.filePath).toContain("bitx-diagnostics-")
		...
		expect(String(writtenPath)).toContain("bitx-diagnostics-test-tas")
		expect(String(writtenContent)).toContain(
			"// Please share this file with Bitx Support (support@roocode.com) to diagnose the issue faster",
		)
