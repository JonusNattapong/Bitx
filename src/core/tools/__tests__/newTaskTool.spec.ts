...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("roo-cline-bitx")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (bitx-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "bitx-nightly"
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("bitx-nightly")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})
	})
