...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".bitx", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .bitx/system-prompt-[mode slug]
...
/**
 * Ensures the .bitx directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	const rooDir = path.join(cwd, ".bitx")
