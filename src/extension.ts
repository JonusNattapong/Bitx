...
import type { CloudUserInfo, AuthState } from "@bitx/types"
import { CloudService, BridgeOrchestrator } from "@bitx/cloud"
import { TelemetryService, PostHogTelemetryClient } from "@bitx/telemetry"
...
/**
 * Check if we should auto-open the Bitx sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening Bitx sidebar for worktree: ${worktreeAutoOpenPath}`)
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize the provider *before* the Bitx Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)

	// Initialize Bitx Cloud service.
	...
	if (process.env.NODE_ENV === "development") {
		const watchPaths = [
			...
			{ path: path.join(context.extensionPath, "node_modules/@bitx/cloud"), pattern: "**/*" },
		]
