...
import type { ProviderName, ModelRecord } from "@bitx/types"
import { modelInfoSchema, TelemetryEventName } from "@bitx/types"
...
async function fetchModelsFromProvider(options: GetModelsOptions): Promise<ModelRecord> {
	...
	switch (provider) {
		...
		case "huggingface":
			...
			break
		case "roo": {
			// Bitx Cloud provider requires baseUrl and optional apiKey
