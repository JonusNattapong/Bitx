import { RooModelsResponseSchema, type ModelInfo, type ModelRecord } from "@bitx/types"
...
/**
 * Fetches available models from the Bitx Cloud provider
 *
 * @param baseUrl The base URL of the Bitx Cloud provider
 * @param apiKey The API key (session token) for the Bitx Cloud provider
...
export async function getRooModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		try {
			...
			if (!parsed.success) {
				console.error("Error fetching Bitx Cloud models: Unexpected response format", data)
				console.error("Validation errors:", parsed.error.format())
				throw new Error("Failed to fetch Bitx Cloud models: Unexpected response format.")
			}
		...
		console.error("[getRooModels] Error fetching Bitx Cloud models:", {
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch Bitx Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch Bitx Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch Bitx Cloud models: No response from server. Check Bitx Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch Bitx Cloud models: ${error.message || "An unknown error occurred."}`)
	}
}
