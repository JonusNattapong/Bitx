...
Bitx s'adapte à votre façon de travailler, pas l'inverse :
...
- **[Documentation](https://docs.roocode.com):** Le guide officiel pour installer, configurer et maîtriser Bitx.
...
Il existe plusieurs façons d'exécuter l'extension Bitx :

### Mode développement (F5)

Pour un développement actif, utilisez le débogage intégré de VSCode :

Appuyez sur `F5` (ou allez dans **Exécuter** → **Démarrer le débogage**) dans VSCode. Cela ouvrira une nouvelle fenêtre VSCode avec l'extension Bitx en cours d'exécution.
...
1.  D'abord, compilez le package VSIX :
    ...
    pnpm vsix
    ```
2.  Un fichier `.vsix` sera généré dans le répertoire `bin/` (par exemple, `bin/roo-cline-bitx-<version>.vsix`).
