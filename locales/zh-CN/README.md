...
了解更多：[使用模式](https://docs.roocode.com/basic-usage/using-modes) • [自定义模式](https://docs.roocode.com/advanced-usage/custom-modes) • [Roomote Control](https://docs.roocode.com/bitx-cloud/roomote-control)
...
- **[文档](https://docs.roocode.com):** 安装、配置和掌握 Bitx 的官方指南。
...
- **[GitHub 问题](https://github.com/RooCodeInc/Bitx/issues):** 报告错误并跟踪开发。
- **[功能请求](https://github.com/RooCodeInc/Bitx/discussions/categories/feature-requests?discussions_q=is%3Aopen+category%3A%22Feature+Requests%22+sort%3Atop):** 有想法吗？与开发人员分享。
...
在 VSCode 中按 `F5`（或转到 **Run** → **Start Debugging**）。这将在运行 Bitx 扩展程序的新 VSCode 窗口中打开。
...
1.  首先，构建 VSIX 包：
    ...
    pnpm vsix
    ```
2.  将在 `bin/` 目录中生成一个 `.vsix` 文件（例如，`bin/roo-cline-bitx-<version>.vsix`）。
...
**请注意**，Bitx, Inc. **不**对与 Bitx 相关的任何代码、模型或其他工具、任何相关的第三方工具或任何由此产生的输出作出任何陈述或保证。您承担使用任何此类工具或输出的**所有风险**；此类工具均按**“原样”**和**“可用”**的基础提供。此类风险可能包括但不限于知识产权侵权、网络漏洞或攻击、偏见、不准确、错误、缺陷、病毒、停机、财产损失或损害和/或人身伤害。您对自己使用任何此类工具或输出负全部责任（包括但不限于其合法性、适当性和结果）。
...
**享受 Bitx！** 无论您是让它保持短绳还是让它自主漫游，我们都迫不及待地想看看您会构建什么。如果您有问题或功能想法，请访问我们的 [Reddit 社区](https://www.reddit.com/r/RooCode/)或 [Discord](https://discord.gg/roocode)。编码愉快！
