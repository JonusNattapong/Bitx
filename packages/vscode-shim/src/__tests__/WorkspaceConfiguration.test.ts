...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("roo-cline-bitx", context)
			...
			setRuntimeConfig("roo-cline-bitx", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("roo-cline-bitx", "testSetting", "testValue")

			expect(getRuntimeConfig("roo-cline-bitx.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("roo-cline-bitx", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("roo-cline-bitx.setting1")).toBe("value1")
			expect(getRuntimeConfig("roo-cline-bitx.setting2")).toBe(42)
			expect(getRuntimeConfig("roo-cline-bitx.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("roo-cline-bitx.defined")).toBe("value")
			expect(getRuntimeConfig("roo-cline-bitx.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("roo-cline-bitx", "setting1", "value1")
			setRuntimeConfig("roo-cline-bitx", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("roo-cline-bitx.setting1")).toBeUndefined()
			expect(getRuntimeConfig("roo-cline-bitx.setting2")).toBeUndefined()
		})
