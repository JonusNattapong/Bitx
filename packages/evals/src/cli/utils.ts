...
import type { ToolUsage } from "@bitx/types"
...
export const resetEvalsRepo = async ({ run, cwd }: { run: Run; cwd: string }) => {
	await execa({ cwd })`git config user.name "Bitx"`
...
export async function copyConversationHistory({
	...
	const extensionStoragePath = "/bitx/.vscode/User/globalStorage/rooveterinaryinc.roo-cline-bitx"
