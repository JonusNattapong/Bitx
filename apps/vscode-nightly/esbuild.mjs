...
import { getGitSha, copyPaths, copyLocales, copyWasms, generatePackageJson } from "@bitx/build"
...
async function main() {
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": '"bitx-nightly"',
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"Bitx-Nightly"',
			...
			setup(build) {
				build.onEnd(() => {
					...
					const generatedPackageJson = generatePackageJson({
						packageJson,
						overrideJson,
						substitution: ["roo-cline-bitx", "bitx-nightly"],
					})
