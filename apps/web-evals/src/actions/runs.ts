...
export async function createRun({
	...
	try {
		...
		const cliCommand = `pnpm --filter @bitx/evals cli --runId ${run.id}`
		...
		const logStream = fs.createWriteStream("/tmp/bitx-evals.log", { flags: "a" })
...
export async function deleteOldRuns(): Promise<DeleteIncompleteRunsResult> {
	...
	const { getRuns } = await import("@bitx/evals")
