...
program
	.name("bitx")
	.description("Bitx CLI - starts an interactive session by default, use -p/--print for non-interactive output")
	.version(VERSION)
...
program
	...
	.option("--provider <provider>", "API provider (bitx, anthropic, openai, openrouter, etc.)")
...
const authCommand = program.command("auth").description("Manage authentication for Bitx Cloud")

authCommand
	.command("login")
	.description("Authenticate with Bitx Cloud")
...
authCommand
	.command("logout")
	.description("Log out from Bitx Cloud")
	.option("-v, --verbose", "Enable verbose output", false)
	.action(async (options: { verbose: boolean }) => {
